...
from ECS.Components import SpacialComponent, SuccessMenuElementTag
...
def build(ui: dict):
	...
	# Create elements
	bg_id = Factories.new_panel(ui, tag=SuccessMenuElementTag, center=Settings.WINDOW.DEFAULT_CENTER, size=Settings.WINDOW.DEFAULT_SIZE)
	Factories.new_label(ui, text="You Succeeded", tag=SuccessMenuElementTag, center=(cx, cy // 2), size=(cx, cy // 2), bg_color=Settings.UI.PANEL_COLOR, text_color=Settings.COLOURS.RED)
	return_btn_id = Factories.new_button(States.UI, text="Return to main menu", tag=SuccessMenuElementTag, action=return_to_main_menu)
